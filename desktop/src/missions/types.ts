...
export interface MissionPlan { id: string; version: number; summary: string; leader_session_id: string; choices: Choice[]; tasks: { key: string; title: string; description: string; depends_on: string[]; acceptance: string[]; kind?: string; internal_steps?: { title?: string; description?: string }[] }[]; limitations: string[]; envelope?: Record<string, unknown>; planning_policy?: Record<string, unknown> }
...
export interface IntegrationConflict { id: string; status: string; classification: string; base_sha: string; ours_sha: string; theirs_sha: string; files?: { path: string; classification: string }[] }
...
export interface MissionSnapshot { schema_version: 1; mission: Mission; plans: MissionPlan[]; tasks: Task[]; sessions: Session[]; assignments: Assignment[]; messages: AgentMessage[]; artifacts: Artifact[]; reviews: Review[]; instructions: Instruction[]; events: MissionEvent[]; worktrees?: WorktreeLease[]; forecasts?: ConflictForecast[]; integrations?: IntegrationAttempt[]; quality_gates?: QualityGateRun[]; approvals?: HumanApproval[]; conflicts?: IntegrationConflict[] }
