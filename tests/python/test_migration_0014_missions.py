...
async def test_mission_migration_empty_and_existing(tmp_path, monkeypatch, upgrade):
    ...
    try:
        ...
        assert 17 in await runner.applied_versions(db.connection)
        assert 18 in await runner.applied_versions(db.connection)
        assert 19 in await runner.applied_versions(db.connection)
